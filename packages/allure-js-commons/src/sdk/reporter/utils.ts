...
import type { LinkTemplateEntry, LinkTemplates } from "./types.js";
...
export const formatLink = (templates: LinkTemplates, link: Link) => {
  const { url, name, type } = link;
  if (isUrl(url)) {
    return link;
  } else {
    const formattedLink = { ...link };
    const { urlTemplate, nameTemplate } = ((templates as any)[type ?? LinkType.DEFAULT] ?? {}) as LinkTemplateEntry;
...
export const formatLinks = (templates: LinkTemplates, links: readonly Link[]) =>
